...
uniform vec3 audioNN;
uniform vec2 classNN;
uniform float streamNN[10];
uniform float classFNN[30];
out vec4 FragColor;
float PI = 3.14159;
float PI2 = 6.28318;
vec3 black = vec3(0.0);
...
vec2 stN(){return (gl_FragCoord.xy / resolution);}
vec2 st(){return (gl_FragCoord.xy / resolution * 2.0 -1.0) * vec2(resolution.x/resolution.y, 1.0);}  
...
float circle(float x,float y,float r,float f) {
    float d=distance(st(),vec2(x, y))/r;
...
//ridged multifractal
float rmf(vec2 st, const in int it) {
    float l = 2.;
    float r = 0.;
    float a = 0.5;
    float f = 1.0;
...
//voronoi fbm
float vfbm(const in vec2 st, const in int it) {
    float n = 0.;
    float a = 0.5;
    float f = 1.0;
    for(int i = 0; i < 32; i++) {
...
//ridged multifractal
float vrmf(vec2 st, const in int it) {
    float l = 2.;
    float r = 0.;
    float a = 0.5;
    float f = 1.0;
...
vec3 sexy(void) {
    ...
    for(int i = 0; i < 80; i++)
    {
       float tTime = float(i) * PI;
        ...
        cr += box(stN()-p.yx, vec2(0.005, 0.01), 0.001, 0.0001) * star;
        cr += box(stN()-p.yx * vec2(1.2, 3.0), vec2(0.005, 0.01), 0.001, 0.0001) * star;
    }
    return vec3(cr * 1.5);
}
